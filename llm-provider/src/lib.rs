use kalosm_language::{
    prelude::{Parse, Schema},
    *,
};
use kalosm_llama::{
    prelude::{ChatModelExt, GenerationParameters},
    *,
};
use kalosm_model_types::FileSource;
use llm_api::{
    ByteStr, ExternSettlementNameCtx, LLMOps, OwnedCtx, SettlementNameCtx, SettlementNamesOutput,
    StatusCode,
};
use log::info;
...
extern "C" fn extern_settlement_names(
    ...
    eprintln!("Staring Settlment names");
    ...
    let user: Box<OwnedCtx> = unsafe { Box::from_raw(user) };
