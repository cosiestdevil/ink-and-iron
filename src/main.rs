...
fn do_things<R: Rng + Clone>(
    ...
    for v_cell in continents_voronoi.iter_cells() {
        ...
        let plate = if let Some(p) = hull_plates.iter().find(|(k, v)| {
...
fn classify_boundaries(cells: &[Cell], plates: &Vec<Plate>) -> Vec<BoundaryEdge> {
    ...
    let mean_edge = 0.5
        * cells
            ...
            .flat_map(|c| {
                c.neighbors
                    .iter()
                    .map(move |&n| (cells[*n as usize].pos - c.pos).length())
            })
            .sum::<f32>()
        / cells
            .iter()
...
fn carve_rivers(cells: &[Cell], h: &mut [f32], threshold: usize) {
    ...
    let mut q: std::collections::VecDeque<CellId> = (0..n)
        .filter(|&i| indeg[i] == 0)
        .map(|i| CellId(i))
        .collect();
...
fn set_sea_level(h: &mut [f32], target_ocean_fraction: f32, is_ocean_mask: Option<&[bool]>) {
    // If you already know which cells must be ocean/land, skip this.
    let mut v = h.to_vec();
    let k = ((target_ocean_fraction.clamp(0.0, 1.0)) * (v.len() as f32)) as usize;
    let (_, lo, _) = v.select_nth_unstable_by(k, |a, b| match (a.is_nan(), b.is_nan()) {
...
fn generate_heightmap(
    ...
    let (d_conv, d_div, d_tr, d_conv_ocean_side, d_conv_land_side) =
...
mod svg {
    ...
    pub fn render_svg<'a, V: Into<usize> + Copy>(
        ...
        cells_height: HashMap<CellId, f32>,
        ...
        blue_piece_path_sstyle.stroke_width = Some(0.1);
        let g = colorgrad::GradientBuilder::new()
//.css("#001a33 0%, #003a6b 25%, #0f7a8a 42%, #bfe9e9 51%, #f2e6c8 55%, #e8d7a1 62%, #a7c88a 68%, #5b7f3a 72%, #8c8f93 85%, #cdd2d8 93%, #ffffff 100%   ").build::<colorgrad::LinearGradient>()?;
.css("#001a33 0%, #003a6b 18%, #0f7a8a 32%, #bfe9e9 42%, #f2e6c8 48%, #e8d7a1 52%, #a7c88a 62%, #5b7f3a 72%, #8c8f93 85%, #cdd2d8 93%, #ffffff 100%   ").build::<colorgrad::LinearGradient>()?;
        for cell in voronoi.iter_cells() {
            ...
            style.fill = Some(if let Some(h) = cells_height.get(&CellId(cell.site())) {
                //let g = colorgrad::preset::yl_gn_bu();
                g.at(*h).to_css_hex()
                // if *h > 0.5 {
                //     "#88CC88".to_string()
                // } else {
    ...
    pub fn render<'a, V: Into<usize> + Copy>(
        ...
        let group = render_svg(&mut svg, scale, voronoi, plates, cells_height)?;
